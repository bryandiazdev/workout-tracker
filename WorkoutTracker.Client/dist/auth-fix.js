...
(function() {
  ...
  if (authToken) {
    ...
    fetch('https://localhost:7250/api/health', {
      headers: {
...
window.fixAuth = function() {
  ...
  fetch('https://localhost:7250/api/health')
    ...
    .catch(error => {
      console.error('Backend API appears to be unreachable:', error.message);
      console.log('Make sure the backend API is running with:');
      console.log('cd WorkoutTracker.Api && dotnet run --launch-profile https');
    });
}; 